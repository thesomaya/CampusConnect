...
const NewChatScreen = (props) => {
  ...
  const userPressed = (userId) => {
    const isBlocker = userBlocks?.[userData.userId]?.[userId] === true;
    const isBlocked = userBlocks?.[userId]?.[userData.userId] === true;

    if (isBlocker || isBlocked) {
      return; // Do nothing if the user is blocked or is the blocker
    }
  ...
  const renderUserItem = (itemData) => {
    const selectedUserId = itemData.item;
    const selectedUserData = users[selectedUserId];

    if (existingUsers && existingUsers.includes(selectedUserId)) {
      return null; // Skip existing users
    }

    const isBlocker = userBlocks?.[userData.userId]?.[selectedUserId] === true;
    const isBlocked = userBlocks?.[selectedUserId] === true;

    const myStyle = isBlocker || isBlocked ? "disabled" : "enabled";
    const onPress =
      isBlocker || isBlocked ? () => {} : () => userPressed(selectedUserId);

    return (
      <DataItem
        title={`${selectedUserData.firstName} ${selectedUserData.lastName}`}
        subTitle={selectedUserData.about}
        image={selectedUserData.profilePicture}
        onPress={onPress}
        type={isGroupChat ? "checkbox" : ""}
        isChecked={selectedUsers.includes(selectedUserId)}
        itemStyle={myStyle}
      />
    );
  };
  ...
  return (
    <PageContainer>
      ...
      {!isLoading && !noResultsFound && users && (
        <FlatList data={Object.keys(users)} renderItem={renderUserItem} />
      )}
